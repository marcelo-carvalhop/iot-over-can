...
static void sendCommandAck(uint8_t originalSubcmd, uint8_t action, uint8_t result) {
  CANMessage ack;
  ack.id = CAN_ID_CONTROL;
  ack.len = 6;
  ack.data[0] = STATUS_OPCODE;
  ack.data[1] = CTRL_SUBCMD_COMMAND_ACK;
  ack.data[2] = NODE_ID;
...
static void handleSensorControl(const CANMessage& rx) {
  ...
  if (action == CTRL_ACTION_OFF) {
    if (state == STATE_FAULT || faultDetected) {
      ...
      sendCommandAck(CTRL_SUBCMD_SENSOR, action, CTRL_ACK_REJECTED);
      return;
    }
    ...
    sendCommandAck(CTRL_SUBCMD_SENSOR, action, CTRL_ACK_APPLIED);
  }
