...
void sendSensorData(uint8_t tick) {
  ...
  uint8_t value = getSensorValueForFaultAnalysis(LOCAL_SENSOR_DEMO_VALUE);
...
void handleCommandAck(const CANMessage& rx) {
  if (rx.len < 6) return;
  if (rx.data[0] != STATUS_OPCODE || rx.data[1] != CTRL_SUBCMD_COMMAND_ACK) return;

  if (state == STATE_GATEWAY) {
    Serial.print("[GW] CMD_ACK node=");
    Serial.print(rx.data[2]);
    Serial.print(" subcmd=0x");
    if (rx.data[3] < 0x10) Serial.print("0");
    Serial.print(rx.data[3], HEX);
    Serial.print(" action=0x");
    if (rx.data[4] < 0x10) Serial.print("0");
    Serial.print(rx.data[4], HEX);
    Serial.print(" result=");
    Serial.println(rx.data[5] == CTRL_ACK_APPLIED ? "APPLIED" : "REJECTED");
  }
}
...
void handleControlCanMessage(const CANMessage& rx) {
  ...
  if (rx.data[0] == STATUS_OPCODE &&
      rx.data[1] == CTRL_SUBCMD_COMMAND_ACK) {
    handleCommandAck(rx);
    return;
  }
