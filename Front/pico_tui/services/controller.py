...
class DomainController:
    ...
    def _subscribe(self) -> None:
        for event_type, handler in (
            ...
            (LocalNodeTelemetryReceived, self._local_node_telemetry),
    ...
    async def _physical_node(self, event: PhysicalNodeReceived) -> None:
        ...
        # ID 0 é a sonda/instrumentação. Nunca entra na topologia funcional.
        if event.parent_node_id == 0:
            self.state.update_gateway(
                can_state=payload.get("CAN", payload.get("STATE", "UNKNOWN")),
            )
            return
    ...
    async def _sensor_status(self, event: SensorStatusReceived) -> None:
        ...
        if "PROFILE" in payload or "PROFILE_ID" in payload:
            values["profile_id"] = str(payload.get("PROFILE", payload.get("PROFILE_ID", ""))).upper()
        elif str(payload.get("MODE", "")).upper() in {"ROTATING", "STRUCTURAL", "SEISMIC"}:
            values["profile_id"] = "VIBRATION"
    ...
    async def _direct_version(self, event: DirectSensorVersionReceived) -> None:
        ...
        self.state.update_sensor(1, 1, protocol_version=event.protocol_version, profile_id="VIBRATION")
    ...
    async def _telemetry(self, event: TelemetryReceived) -> None:
        ...
        if sample.mode.value in {"ROTATING", "STRUCTURAL", "SEISMIC"} or any(
            value is not None
            for value in (
                sample.rms,
                sample.kurtosis,
            ...
            telemetry_values["profile_id"] = "VIBRATION"
        self.state.update_sensor(
            sample.parent_node_id,
            sample.child_id,
            **telemetry_values,
    ...
    async def _local_node_telemetry(self, event: LocalNodeTelemetryReceived) -> None:
        node = self.state.ensure_node(event.parent_node_id, "CAN_NODE")
        capabilities = set(node.capabilities)
        capabilities.update({"CAN", "LOCAL_SENSOR"})
        if event.profile_id == "DEMO_BYTE":
            capabilities.add("LOCAL_SENSOR_DEMO")
        self.state.update_node(
            event.parent_node_id,
            node_type="CAN_NODE",
            capabilities=capabilities,
            local_sensor_profile=event.profile_id,
            local_sensor_value=event.value,
            local_sensor_enabled=event.enabled,
            local_sensor_last_round=event.round_number,
            local_sensor_last_seen_monotonic=time.monotonic(),
            legacy_last_value=f"0x{event.value:02X}",
            legacy_last_round=event.round_number,
            rx_count=node.rx_count + 1,
            status=NodeStatus.ONLINE,
        )
