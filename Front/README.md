...
```bash
iot-over-iot-over-can-tui --demo
```

### Detecção automática

```bash
iot-over-iot-over-can-tui --port /dev/ttyACM0 --mode auto
```

### Sensor direto

```bash
iot-over-iot-over-can-tui --port /dev/ttyACM0 --mode sensor
```

### Gateway CAN FD

```bash
iot-over-iot-over-can-tui --port /dev/ttyUSB0 --mode gateway
```

### Windows

```powershell
iot-over-iot-over-can-tui --port COM5 --mode sensor
```
...
TELEMETRY PERIOD <ms>
FFT ONCE
SIMULATE ON/OFF removido da baseline enxuta
ACQ POLLING
DTC
DTC CLEAR
NET
...
```bash
iot-over-iot-over-can-tui --port /dev/ttyACM0 --mode sensor --security-mode presence
```
...
A TUI não exige mais uma porta serial no início. Ela pode ser aberta com nenhum ESP32, Pico ou gateway conectado:

```bash
iot-over-can-tui
```

Nesse estado, a interface abre normalmente em `DISCONNECTED` e permite navegar pelo console. Para conectar depois:
...
O alias legado `pico-tui` permanece temporariamente apenas para compatibilidade.
...
Depois execute sem reinstalar o pacote:

```bash
../../../scripts/run_tui.sh
```

## Perfis e capacidades de nó

A TUI diferencia três entidades:

```text
Rede CAN
Módulo CAN físico
Sensor wireless associado
```

O módulo CAN possui tela própria. O sensor local `DEMO_BYTE=0xAA` pertence ao módulo físico.

Telas de sensor são orientadas por `PROFILE_ID`. Na baseline, Telemetria/FFT só aparecem no menu quando o sensor selecionado tem `PROFILE=VIBRATION`.


## Segurança v0.12

O arquivo padrão é `~/.config/iot-over-can/security.json` e, em POSIX, deve usar `0600`. O campo `device_admin_token` corresponde ao token compilado no firmware do Pico. A TUI nunca grava esse token no EventLog.

`otp` está fail-closed nesta versão porque a validação antiga por prefixo/tamanho não comprovava um OTP Yubico. Use `presence` para a política operacional atual; FIDO2/OTP criptográfico será uma integração posterior.
