# Protocolo Textual de Referência do Gateway
...
```text
GW_VERSION FIRMWARE=0.5.0 PROTOCOL=1.0
```
...
```text
NODE NODE=20 TYPE=SENSOR_BRIDGE STATE=ONLINE FIRMWARE=0.3.0 \
PROTOCOL=1.0 CAN=ONLINE WIFI=ONLINE UPTIME_MS=55120
```

## Sensor wireless

```text
SENSOR NODE=20 CHILD=1 UUID=0x10A4 STATE=ONLINE MODE=ROTATING \
ACQ=POLLING QUALITY=REAL NET=BOUND
```
...
```text
[GW] HEARTBEAT lider=4 rodada=18 sensores_ativos=3 modo=2 periodo=1000 ms
