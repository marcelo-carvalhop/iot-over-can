...
from pico_tui.core.models import AppState, SensorNode
...
class NetworkTreePanel(Vertical):
    """Árvore hierárquica Gateway → nós físicos → sensores wireless."""
    ...
    def refresh_state(self, state: AppState) -> None:
        ...
        fingerprint = tuple(
            (parent, node.node_type, tuple((child, sensor.wireless_uuid) for child, sensor in sorted(node.sensors.items())))
        ...
        for parent_id, node in sorted(state.nodes.items()):
            ...
            label = f"{marker} Node {parent_id:02d} — {node.node_type}"
            ...
            for child_id, sensor in sorted(node.sensors.items()):
                ...
                parent.add(
                    f"{selected} {quality} {sensor.logical_id}{uuid}",
        ...
        for node in tree.root.children:
            ...
            if data[0] == "node":
                ...
                if physical:
                    ...
                    node.set_label(f"{marker} Node {physical.parent_node_id:02d} — {physical.node_type}")
                for child_node in node.children:
                    ...
                    if isinstance(child_data, tuple) and child_data[0] == "sensor":
                        ...
                        if sensor:
                            ...
                            uuid = f" {sensor.wireless_uuid}" if sensor.wireless_uuid else ""
                            selected = "▶" if state.selected_logical_id == sensor.logical_id else " "
                            child_node.set_label(f"{selected} {quality} {sensor.logical_id}{uuid}")
...
class TelemetryPanel(Vertical):
    ...
    def compose(self) -> ComposeResult:
        yield Label("TELEMETRIA", classes="panel-title")
...
class QuickStatusPanel(Vertical):
    ...
    def refresh_state(self, state: AppState, sensor: SensorNode | None) -> None:
        ...
        self.query_one("#quick-status", Static).update(
            f"Gateway: {gateway.can_state}/{gateway.wifi_state}\n"
